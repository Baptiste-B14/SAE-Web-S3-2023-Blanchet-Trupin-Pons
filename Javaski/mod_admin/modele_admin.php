...
class ModeleAdmin extends Connexion{
	...
	public function accepterUser($idUser) {
		...
		$pdo_req->execute(['idU'=>htmlspecialchars($idUser),'pseudo'=>htmlspecialchars($user[0]["pseudo"]), 'identifiant'=>htmlspecialchars($user[0]["identifiant"]), 'motdepasse'=>htmlspecialchars($user[0]["motdepasse"]), 'courriel'=>htmlspecialchars($user[0]["courriel"]), 'cheminVersPhoto'=>htmlspecialchars($user[0]["cheminVersPhoto"]), 'droits'=>htmlspecialchars($user[0]["droits"])]);
	}
